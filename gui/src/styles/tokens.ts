// styles/tokens.ts  —  Single source-of-truth design tokens
...
export const pvalColor = (p: number): string =>
  p < 0.001 ? T.red : p < 0.05 ? T.amber : T.green;

export const cellColor = (val: number, max: number): string => {
  if (val === 0) return T.bg0;
  return `rgba(74,158,255,${(0.15 + (val / max) * 0.7).toFixed(3)})`;
};
...
.select { background: ${T.bg1}; border: 1px solid ${T.border}; color: ${T.text}; border-radius: 6px; padding: 7px 10px; font-size: 12px; cursor: pointer; }
